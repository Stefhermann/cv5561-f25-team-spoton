...
def batch_crop_dice(img, yolo_res, res_shape=(128, 128)):
    ...
    assert dice_cropped.shape == (
        n_dice,
        res_shape[0],
        res_shape[1],
        3,
    )  # (die, x, y, channel) -> intensity
    return dice_cropped
...
def posterize(dice_cropped, k_means, palette):
    ...
    posterized = einops.rearrange(
        flat_colors, "(N H W) C -> N H W C", N=N, H=H, W=W, C=C
    )
...
def dice_to_histograms(dice_cropped, k_means):
    ...
    flat_in = einops.rearrange(dice_cropped, "N H W C -> (N H W) C")
    flat_out = k_means.predict(flat_in)
    color_classes_flattened = einops.rearrange(
        flat_out, "(N H W) -> N (H W)", N=n_dice, H=H, W=W
    )
    ...
    for die_idx in range(n_dice):  # TODO: vectorize?
        ...
        histograms[die_idx, :] = np.bincount(
            color_classes_flattened[die_idx], minlength=palette_size
        )
...
def load_labeled_dice():
    ...
    with open(labels_path, "r") as label_file:
        for line in label_file:
            ...
            if (
                id not in img_dict
                or player_label not in legal_player_labels
                or value_label not in legal_value_labels
            ):
...
def fit_player_colors(
    obb_model,
    unsupervised_dice_imgs,
    player_dice_imgs,
    palette_size=16,
    n_extra_clusters=1,
):
    ...
    for img_idx in range(N_unsupervised):
        ...
        # res = res[0]
        # print(res)
        curr_dice_cropped = batch_crop_dice(
            unsupervised_img, res
        )  # (n_dice_in_img, H, W, C)
...
class RecognitionModel:
    ...
    def train_player_vocab(self, unsupervised_imgs, player_imgs, n_extra_clusters=1):
        self.color_k_means, self.player_k_means, self.cluster_to_player = (
            fit_player_colors(
                self.obb_model,
                unsupervised_imgs,
                player_imgs,
    ...
    def predict(self, frame, obb_confidence=0.25, value_confidence=0.55):
        ...
        dice_players = predict_player_colors(
            dice_cropped,
            self.color_k_means,
            self.player_k_means,
            self.cluster_to_player,
        )
        dice_values, dice_val_conf = predict_die_value(
